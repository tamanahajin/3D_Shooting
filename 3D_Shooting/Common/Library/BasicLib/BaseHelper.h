...
namespace shooting {
	...
	struct 	Util {
		...
		*/
		...
		//--------------------------------------------------------------------------------------
		/*!
		@brief	Enumの値を文字列に変換する
		@param[in]	value	Enumの値
		@return	文字列に変換されたEnumの値
		*/
		//--------------------------------------------------------------------------------------
		template <typename T>
	...
	*/
	...
	protected:
		...
		//構築と破棄
		Dx12Shader(const std::wstring& fileName) : Dx12ShaderResource(), m_fileName(fileName) {}
		virtual ~Dx12Shader() {}
	...
	//これより個別シェーダー
	//シェーダ宣言用マクロ
#define DECLARE_DX12SHADER(ShaderName) \
class ShaderName : public Dx12Shader<ShaderName> { \
public: \
    ShaderName(); \
};
