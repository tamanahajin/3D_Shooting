...
namespace shooting {
	...
	// 日本語： Direct3D 12をサポートする最初に利用可能なハードウェアアダプタを取得するためのヘルパー関数。
	// 見つからない場合、*ppAdapterはnullptrに設定されます。
		...
		if (SUCCEEDED(pFactory->QueryInterface(IID_PPV_ARGS(&factory6))))
		{
				...
				++adapterIndex)
			{
				...
				if (desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE)
				{
					...
					// 日本語： Basic Render Driverアダプタを選択しないでください。
					// ソフトウェアアダプタが必要な場合は、コマンドラインで「/warp」を渡します。
					continue;
				}

				// Check to see whether the adapter supports Direct3D 12, but don't create the
				// actual device yet.
				// 日本語： アダプタがDirect3D 12をサポートしているかどうかを確認しますが、実際のデバイスはまだ作成しません。
	...
	// 日本語： 提供されたコマンドライン引数を解析するためのヘルパー関数。
	_Use_decl_annotations_
